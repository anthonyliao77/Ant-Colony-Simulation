...
public class Main extends Application {
    ...
    public Main() {
        DispersalPolicy dispersalPolicy = new DefaultDispersalPolicy(0.9f, 0.3f);
    ...
    public class DefaultDispersalPolicy implements DispersalPolicy {
        private final float f; // Fall-off konstant
        private final float k; // Keep konstant

        public DefaultDispersalPolicy(float f, float k) {
            this.f = f;
            this.k = k;
        }
            ...
            if (!w.isObstacle(p)) {
                for (int dx = -1; dx <= 1; dx++) {
                    for (int dy = -1; dy <= 1; dy++) {
                        if (dx == 0 && dy == 0) { // skip because not neighbour
                            continue;
                        }
                        int nx = Math.max(0, Math.min(w.getWidth() - 1, x + dx)); // kollar om den är utanför
                        int ny = Math.max(0, Math.min(w.getHeight() - 1, y + dy));

                        Position cell = new Position(nx, ny);
                        foragingPheromone += w.getForagingStrength(cell);
                        foodPheromone += w.getFoodStrength(cell);
                    }
                }

                foragingPheromone = ((1 - this.k) * foragingPheromone) / 8 + this.k * w.getForagingStrength(p);
                foodPheromone = ((1 - this.k) * foodPheromone) / 8 + this.k * w.getFoodStrength(p);
            }

            return new float[]{foragingPheromone * this.f, foodPheromone * this.f};
        }
    }
